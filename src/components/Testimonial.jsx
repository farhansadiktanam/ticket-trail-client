...
export default function Testimonial() {
  return (
    <main>
      {/* Page header */}
      <section className="bg-[#0B1320] px-6 py-16 text-center lg:px-12">
        ...
        <p className="mx-auto mt-3 max-w-xl text-sm text-[#9AA6B8] sm:text-base">
          Real feedback from people who&apos;ve booked bus, train, launch, and
          flight tickets through TicketTrail.
        </p>

        <div className="mx-auto mt-10 grid max-w-2xl grid-cols-3 divide-x divide-[#1E2A3D]">
          {stats.map((stat) => (
            <div key={stat.label} className="px-4">
              <p className="text-2xl font-bold text-white sm:text-3xl">
                {stat.value}
              </p>
              <p className="mt-1 text-xs text-[#8A96A8] sm:text-sm">
                {stat.label}
              </p>
            </div>
          ))}
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t) => {
            ...
            return (
              ...
              >
                <Quote className="h-6 w-6 text-[#F2B134]/40" />
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0B1320] text-sm font-semibold text-[#F2B134]">
                      {getInitials(t.name)}
                    </span>
                    <div>
                      ...
                      <p className="text-xs text-[#5C6779]">{t.location}</p>
                    </div>
                  </div>
      ...
      {/* CTA band */}
      <section className="bg-[#0B1320] px-6 py-14 text-center lg:px-12">
        <h2 className="text-2xl font-bold text-white sm:text-3xl">
          Ready to start your own journey?
        </h2>
        <p className="mx-auto mt-3 max-w-md text-sm text-[#9AA6B8]">
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-4">
          ...
          <Link
            href="/contact"
            className="text-sm font-semibold text-[#C7D0DD] transition-colors hover:text-white"
          >
            Share Your Story
          </Link>
        </div>
